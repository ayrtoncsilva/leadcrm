...
export async function create(req, res) {
  try {
    const { data, nome, origem, qualificado, ativado, respondeu, vendido } = req.body;
    if (!nome?.trim() || !data) {
      return res.status(400).json({ error: 'Nome e data são obrigatórios.' });
    }
    const id = await Contact.create({ data, nome: nome.trim(), origem, qualificado, ativado, respondeu, vendido });
